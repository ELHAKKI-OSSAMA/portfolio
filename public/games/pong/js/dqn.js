...
forward(s){let x=this.l1.forward(s).map(relu);x=this.l2.forward(x).map(relu);return this.l3.forward(x);}
...
class PongAgent{constructor(si=6,hi=64,ai=3){this.online=new QNet(si,hi,ai);this.target=this.online.clone();this.mem=[];this.eps=1.0;this.steps=0;}
act(s){if(rnd()<this.eps)return Math.floor(rnd()*3);return argmax(this.online.forward(s));}
push(s,a,r,ns,done){if(this.mem.length>5000)this.mem.shift();this.mem.push({s,a,r,ns,done});}
train(){if(this.mem.length<128)return;for(let k=0;k<4;k++){const{s,a,r,ns,done}=this.mem[Math.floor(rnd()*this.mem.length)];const tq=this.target.forward(ns);const t=done?r:r+0.95*Math.max(...tq);this.online.train(s,a,t);}this.steps++;if(this.eps>0.02)this.eps*=0.9995;if(this.steps%100===0)this.target=this.online.clone();}
getQ(s){return this.online.forward(s);}}
