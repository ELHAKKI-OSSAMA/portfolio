...
class PongAgent{constructor(si=6,hi=64,ai=3){this.online=new QNet(si,hi,ai);this.target=this.online.clone();this.mem=[];this.eps=1.0;this.steps=0;this.episode=0;this.lastActs=null;}
...
train(){if(this.mem.length<128)return;for(let k=0;k<4;k++){const{s,a,r,ns,done}=this.mem[Math.floor(rnd()*this.mem.length)];const tq=this.target.predict(ns);const t=done?r:r+0.95*Math.max(...tq);this.online.train(s,a,t);}this.steps++;if(this.eps>0.02)this.eps*=0.9995;if(this.steps%100===0)this.target=this.online.clone();}
