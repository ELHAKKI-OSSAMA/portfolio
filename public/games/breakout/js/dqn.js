...
// Optimised: gamma 0.99, eps decay 0.9995, train 8x per step, target sync every 50 steps
...
train(){if(this.mem.length<256)return;for(let k=0;k<8;k++){const{s,a,r,ns,done}=this.mem[Math.floor(rnd()*this.mem.length)];const tq=this.target.forward(ns).q;const t=done?r:r+0.99*Math.max(...tq);this.online.train(s,a,t);}this.steps++;if(this.eps>0.01)this.eps*=0.9995;if(this.steps%50===0)this.target=this.online.clone();if(this.steps%10===0)this.epsHist.push(this.eps);}
