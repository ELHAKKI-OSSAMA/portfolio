...
reset(){this.x=120+rnd()*60;this.y=GH/2+(rnd()-0.5)*100;this.angle=0;this.vx=1;this.vy=0;this.alive=true;this.frames=0;this.out=[0,0,0];}
step(asts){if(!this.alive)return;const rays=getRays(this,asts);const inp=[...rays];const out=this.genome.forward(inp);this.out=out;const turnL=out[0],turnR=out[1],thrust=out[2];this.angle+=(turnR-turnL)*0.07;const spd=1+thrust*3;this.vx=Math.cos(this.angle)*spd;this.vy=Math.sin(this.angle)*spd;this.x+=this.vx;this.y+=this.vy;// wrap Y
...
function loop(){requestAnimationFrame(loop);
  if(_paused){draw();return;}for(let i=0;i<simSpeed;i++)simStep();draw();document.getElementById('s-gen').textContent=neat.generation;document.getElementById('s-alive').textContent=ships.filter(s=>s.alive).length;document.getElementById('s-best').textContent=bestFrames;document.getElementById('s-sp').textContent=neat.species.length;}
loop();
