...
step(asts){if(!this.alive)return;
  ...
  this.frames++;
  for(const a of asts){
    if(Math.hypot(this.x-a.x,this.y-a.y)<a.r+8){this.alive=false;return;}
...
const neat=new NEAT();
let asteroids=Array.from({length:18},()=>new Asteroid());
let ships=neat.genomes.map((g,i)=>new Ship(g,i));
let bestFrames=0;
...
function simStep(){
  ...
  if(ships.every(s=>!s.alive)){
    const best=ships.reduce((a,b)=>b.frames>a.frames?b:a);
...
function loop(){
  ...
  document.getElementById('s-best').textContent=bestFrames;
